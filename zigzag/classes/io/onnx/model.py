...
class ONNXModelParser:
    ...
    # @param onxx_model
    # @param mapping_path
    def __init__(self, onnx_model, mapping_path, precision) -> None:
        ...
        if isinstance(precision, str):
            self.precision_path = precision
            self.precision = None
        elif isinstance(precision, dict):
            self.precision = precision
            self.precision_path = None
        else:
            raise TypeError(f"Given precision is of type {type(mapping_path)}.")
        self.workload = None
    ...
    def run(self):
        ...
        if not self.precision:
            self.precision = self.parse_precision_from_path(self.precision_path)
